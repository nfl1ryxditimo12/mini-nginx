...
// add &(reference) to curr_server
void ws::Repository::set_repository(const ws::Server& curr_server, const ws::Request& request) {
  ...
  location_map_type::const_iterator curr_location = curr_server.get_location_map().find(request.get_uri());
...
// why params are pointer? (changed to const &)
// why all param has `_' at front of their name? (removed `_')
...
void ws::Repository::get_repository(const ws::Configure& conf, const ws::Request& request) {
  for (
